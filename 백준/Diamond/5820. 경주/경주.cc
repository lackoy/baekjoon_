...
using namespace std;
using ll = long long;
const int MXN = 200000, MXK = 1000000, INF = 1e9;
int sub_sz[MXN+5], min_depth[MXK+5];
bool vis[MXN+5];
vector<int> cur_tree;
...
int dnc(int cur){
    ...
    int ct = get_cent(cur, -1, thr/2);
    vis[ct] = 1;
    int ret = INF;

    for(const auto& a : cur_tree)
        min_depth[a] = INF;
